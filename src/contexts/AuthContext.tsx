...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, userData: { name: string }) => Promise<{ success: boolean; error?: string; userId?: string }>;
  signOut: () => Promise<{ success: boolean; error?: string }>;
  updateProfile: (userId: string, updates: Partial<Pick<UserProfile, 'name' | 'avatar_url'>>) => Promise<{ success: boolean; error?: string }>;
  logout: () => Promise<{ success: boolean; error?: string }>;
  // Permission helpers
  getModulePermissions: (module: string) => ModulePermission | undefined;
  can: (module: string, action: 'create' | 'edit' | 'delete') | 'manage_users' | 'manage_roles') => boolean;
...
export const AuthProviderComponent: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadUserAndPermissions = useCallback(async (supabaseUser: SupabaseUser | null) => {
    if (supabaseUser) {
      try {
        const userProfile = await userService.getUserProfile(supabaseUser.id);
        if (userProfile) {
          setUser(userProfile);
          reactQueryClient.setQueryData(['userProfile', supabaseUser.id], userProfile);

          const userPermissions = await permissionsService.getUserPermissions(supabaseUser.id);
          setPermissions(userPermissions);
          reactQueryClient.setQueryData(['userPermissions', supabaseUser.id], userPermissions);
        } else {
          ...
          console.warn(`User profile not found for ${supabaseUser.id}. User might need to complete profile setup or trigger is pending.`);
  ...
  useEffect(() => {
    ...
    const { data: authListener } = onAuthStateChange(async (_event, newSession) => {
      ...
      reactQueryClient.invalidateQueries(['userProfile', newSession?.user?.id]);
      reactQueryClient.invalidateQueries(['userPermissions', newSession?.user?.id]);
    });
  ...
  const signUpMutation = useMutation(
    ...
    ({ email, password, name }: { email: string; password: string; name: string }) =>
      signUpWithSupabase({ email, password, options: { data: { full_name: name } } }),
    {
      onSuccess: (data) => {
  ...
  const updateProfileMutation = useMutation(
    ({ userId, updates }: { userId: string, updates: Partial<Pick<UserProfile, 'name' | 'avatar_url'>>}) =>
      userService.updateSubUser(userId, updates), // Using updateSubUser as it updates user_profiles
    {
      onSuccess: (updatedProfile) => {
        if (updatedProfile) {
          setUser(updatedProfile);
          reactQueryClient.setQueryData(['userProfile', updatedProfile.user_id], updatedProfile);
        }
      },
      onError: (error: any) => {
  ...
  const handleSignUp = async (email: string, password: string, userData: { name: string }) => {
    try {
      const {data, error} = await signUpMutation.mutateAsync({ email, password, name: userData.name });
  ...
  const handleUpdateProfile = async (userId: string, updates: Partial<Pick<UserProfile, 'name' | 'avatar_url'>>) => {
    try {
  ...
  const getModulePermissions = useCallback((module: string): ModulePermission | undefined => {
    if (permissions?.isSuperAdmin) {
      return { view_type: 'all', can_create: true, can_edit: true, can_delete: true };
    }
    return permissions?.modules?.[module];
  }, [permissions]);

  const can = useCallback((module: string, action: 'create' | 'edit' | 'delete' | 'manage_users' | 'manage_roles'): boolean => {
    if (permissions?.isSuperAdmin) return true;

    // Specific checks for meta-permissions like 'manage_users'
    if (action === 'manage_users') return permissions?.modules?.['settings_users']?.can_create || permissions?.modules?.['settings_users']?.can_edit || permissions?.modules?.['settings_users']?.can_delete || false;
    if (action === 'manage_roles') return permissions?.modules?.['settings_roles']?.can_create || permissions?.modules?.['settings_roles']?.can_edit || false;
  ...
  const canView = useCallback((module: string, viewTypeToMatch?: 'all' | 'assigned'): boolean => {
    if (permissions?.isSuperAdmin) return true;
    const modulePerms = getModulePermissions(module);
  ...
  const value: AuthContextType = {
    ...
    session,
    permissions,
    loading: loadingInitial || signInMutation.isLoading || signUpMutation.isLoading || signOutMutation.isLoading,
